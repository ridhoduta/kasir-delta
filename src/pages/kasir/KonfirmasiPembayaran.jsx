...
import { createMidtransToken } from '../../api/midtransApi';
...
const KonfirmasiPembayaran = () => {
  ...
  const handleBayar = async () => {
    ...
    try {
      ...
      const savedPesanan = response.data;

      // 2. Dapatkan Snap Token dari backend
      const midtransResponse = await createMidtransToken(savedPesanan.pesananid);
      const snapToken = midtransResponse.snap_token;

      // 3. Panggil Midtrans Snap
      window.snap.pay(snapToken, {
        onSuccess: function (result) {
          console.log('Payment Success:', result);
          navigate('/nota', { state: { pesanan: savedPesanan } });
        },
        onPending: function (result) {
          console.log('Payment Pending:', result);
          navigate('/nota', { state: { pesanan: savedPesanan, status: 'pending' } });
        },
